...
# Add rate limiter middleware AFTER CORS
...
async def get_vehicle_info(
    ...
    if is_cached:
        ...
        logger.info(f"Cache hit for registration number: {vrn}")
    ...
    logger.info(f"Processing request for registration number: {vrn}")
    ...
    while retry_count <= max_retries:
        try:
            ...
            if resp.status_code == 400:
                logger.warning(f"Bad request for VRN {vrn}: {error_detail}")
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, 
                    detail=f"Bad request: {error_detail}"
                )
            elif resp.status_code == 404:
                logger.info(f"Vehicle not found for VRN {vrn}")
                raise HTTPException(
                ...
                if retry_count <= max_retries:
                    ...
                    logger.warning(f"Rate limited by DVLA API. Retrying in {wait_time} seconds.")
                ...
                if retry_count <= max_retries:
                    wait_time = Config.RETRY_BACKOFF_FACTOR * (2 ** (retry_count - 1))
                    logger.warning(f"DVLA API server error. Retrying in {wait_time} seconds.")
                    await asyncio.sleep(wait_time)
                    continue
                else:
                    logger.error(f"DVLA API server error and max retries reached: {error_detail}")
                    raise HTTPException(
                ...
                # Other errors
                logger.error(f"Unexpected error from DVLA API: {resp.status_code} - {error_detail}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
